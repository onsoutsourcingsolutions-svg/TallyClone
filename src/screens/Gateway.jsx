...
export function Gateway() {
  ...
  return (
    <div style={{ maxWidth: 1280, margin: '0 auto' }}>
      ...
      {/* ---------- TOP KPI ROW — v1.11.41 FIX: sales total now correct, stock import NOT in sales, invoice vs taxable ---------- */}
      <div className="kpis" style={{ marginBottom: 14, gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))' }}>
        ...
        <div className="kpi" style={{ cursor: 'pointer', borderColor: 'var(--gold)', borderStyle: 'dashed' }} onClick={() => setView({ name: 'reports', which: 'stock' })} title="Stock value — separate, NOT part of sales — stock_journal does NOT touch sales"><div className="k" style={{ color: 'var(--ink-dim)' }}>Stock value (Not Sales)</div><div className="v">{fmt(stockVal)}</div><div className="s">{d?.stock?.count || 0} items · {fmtQty(d?.stock?.totalQty)} qty · Separate · Click ↓</div></div>
        <div className="kpi" style={{ cursor: 'pointer', borderColor: 'var(--gold)', background: 'linear-gradient(180deg, rgba(212,175,55,0.12), rgba(0,0,0,0.8))' }} onClick={() => setView({ name: 'daybook' })} title="Sales FY taxable — sales ledger only, stock import NOT included. Invoice FY incl GST is separate"><div className="k">Sales FY (Taxable)</div><div className="v" style={{ color: 'var(--gold-hi)' }}>{fmt(d?.sales?.fy)}</div><div className="s">Inv FY {fmt(d?.sales?.invoiceFY)} incl GST · {d?.sales?.countFY||0} bills · Click ↓</div></div>
        <div className="kpi" style={{ cursor: 'pointer', borderColor: 'var(--gold-hi)', background: 'linear-gradient(180deg, rgba(212,175,55,0.18), #000)' }} onClick={() => setView({ name: 'daybook' })} title="Invoice value FY incl GST — this is 927954 you mentioned — party Dr total excl stock valuation"><div className="k" style={{ color: 'var(--gold-hi)' }}>Invoice Value FY (incl GST)</div><div className="v" style={{ color: 'var(--gold-hi)' }}>{fmt(d?.sales?.invoiceFY)}</div><div className="s">Taxable {fmt(d?.sales?.fy)} + GST {fmt((d?.sales?.invoiceFY||0)-(d?.sales?.fy||0))} · Click ↓</div></div>
        <div className="kpi" style={{ cursor: 'pointer' }} onClick={() => setView({ name: 'daybook' })} title="Sales this month taxable + invoice"><div className="k">Sales Month (Taxable)</div><div className="v">{fmt(salesM)}</div><div className="s">Inv {fmt(d?.sales?.invoiceMonth)} · {d?.sales?.countMonth||0} bills · Click ↓</div></div>
