  ...
  return (
    <div className="card" style={{ borderColor: 'var(--gold)', background: 'linear-gradient(180deg, #1a170b, #000000)', borderLeft: '4px solid var(--gold)' }}>
      ...
      <p className="muted" style={{ fontSize: 13, margin: '0 0 10px', color: 'var(--ink-dim)' }}>
        <b style={{ color: 'var(--gold-hi)' }}>v1.11.48 — EDIT VOUCHER FIXED + SALES EDIT + COGS EXCLUDED ✓ 73 inv = 927956 incl GST matched:</b> FY25-26 20 inv = 374616, FY26-27 53 inv = 553340, Grand = 927956. <b style={{ color: 'var(--gold-hi)' }}>Amount = Invoice incl GST excl COGS (party Dr) — NOT debit which includes stock valuation.</b> Edit any sales invoice from Voucher list → now shows items/qty/rate (items stored even for PI/Proforma, only Tax Invoice affects stock). <b style={{ color: 'var(--ink)' }}>Upload ONE Excel where each sheet = one bill</b> → auto-booked DD/MM/YYYY, stock matched live (only Tax Invoice affects stock, PI/Proforma NO but editable). <b style={{ color: 'var(--gold)' }}>Hover/click stock item</b> for history. <b style={{ color: 'var(--gold-hi)' }}>Download buttons everywhere.</b>
      </p>
      <div style={{ border: '1px solid var(--gold-line)', borderRadius: 6, padding: '8px 10px', marginBottom: 10, background: 'rgba(212,175,55,0.08)', fontSize: 12 }}>
        <b style={{ color: 'var(--gold-hi)' }}>Download reconciliation:</b> <a href="/templates/sales-927956-verified.xlsx" style={{ color: 'var(--gold-hi)', textDecoration: 'underline' }}>Your 73 invoices 927956 Verified Excel</a> | <a href="/templates/sales-927956-import-template.xlsx" style={{ color: 'var(--gold-hi)', textDecoration: 'underline' }}>Import Template (taxable rate → 927956 incl GST)</a> | <a href="/api/export/sales" style={{ color: 'var(--gold-hi)', textDecoration: 'underline' }}>Live Sales from DB</a> | <a href="/api/export/dashboard" style={{ color: 'var(--gold-hi)', textDecoration: 'underline' }}>Dashboard Excel</a>
      </div>

      {/* TOTAL SALE DONE — v1.11.41 FIX: stock import does NOT inflate sales, fix fy+month double count bug */}
      <div style={{ border: '1px solid var(--gold-line)', borderRadius: 6, padding: '8px 10px', marginBottom: 10, background: 'rgba(212,175,55,0.10)', fontSize: 12, color: 'var(--ink-dim)', borderLeft: '3px solid var(--gold-hi)' }}>
        <b style={{ color: 'var(--gold-hi)' }}>v1.11.48 FIX — Edit voucher + Sales edit + COGS excluded:</b> <b style={{ color: 'var(--gold-hi)' }}>Cannot edit sales invoice detail — FIXED:</b> item_entries now always stored even for PI/Proforma (only Tax Invoice affects stock, PI no stock but editable). <b style={{ color: 'var(--gold-hi)' }}>Figures in sale including COGS — FIXED:</b> Amount in Voucher list/DayBook/SalesExcel = invoice_total (sundry_debtors) excl stock valuation, debit incl COGS shown only as tooltip. Dashboard FY 927956 = invoice incl GST excl COGS. Stock import (stock_journal) never in sales.
      </div>
